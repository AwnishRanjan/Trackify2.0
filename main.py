...
def initilise_deepsort():
    cfg_deep = get_config()
    cfg_deep.merge_from_file("deepsort/configs/deep_sort.yaml")
...
def draw_boxes(frame, bbox_xyxy, draw_trails, identities=None, categories=None, offset=(0, 0)):
    ...
    for i, box in enumerate(bbox_xyxy):
        ...
        x1 += offset[0]
        y1 += offset[1]  # Fix offset for y-axis
        x2 += offset[0]
        y2 += offset[1]  # Fix offset for y-axis
...
def run(
        ...
        iou_thres=0.45,  # NMS IOU thresholdƒ
    ...
    if is_url and is_file:
        source = check_file(source)  # download the file if url and file 
    ...
    # Run inference
    model.warmup(imgsz=(1 if pt or model.triton else bs, 3, *imgsz))  # warmup memory allocation and reserve 
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        # Process predictions
        for i, det in enumerate(pred):  # per image  ,, det = detection 
            seen += 1
            ...
            if len(det):
                ...
                # pass thorugh>>>>>>>>>>>>>> deepsort(yolo_result)  
                xywh_bboxs = []
                confs = []
                oids = []   # object ids 
                outputs = []

                for *xyxy, conf, cls in reversed(det):   # conf = confidence score , cls= class name 
                    x1 , y1, x2 , y2 = xyxy
                    x1, y1, x2, y2 = int(x1), int(y1), int(x2), int(y2)

                    cx, cy = int((x1+x2)/2), int((y1+y2)/2)   # center co ordinates 

                    bbox_width = abs(x1-x2)   # find w and h 
                    bbox_height = abs(y1-y2)

                    xcycwh = [cx, cy, bbox_width, bbox_height]  #(center(x,y) w , h )
                    xywh_bboxs.append(xcycwh)
                    conf = math.ceil(conf*100)/100     # round off confidence to 2 decimal palces 
                    confs.append(conf)

                    oids.append(int(cls))
                
                xywh_s = torch.Tensor(xywh_bboxs)
                confidense_scores = torch.Tensor(confs)

                outputs = deepsort.update(xywh_s,confidense_scores, oids, ims)
...
def main(opt):
    opt.vid_stride = 2 # check_requirements(exclude=('tensorboard', 'thop'))
    run(**vars(opt))
